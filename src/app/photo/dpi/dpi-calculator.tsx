...
import { calculateDPI, COMMON_PRINT_SIZES, COMMON_DPI } from "@/lib/converters/photo/dpi";
...
export function DPICalculator() {
  ...
  const result = calculateDPI(parseFloat(printWidth) || 0, parseFloat(printHeight) || 0, parseInt(dpi) || 0);
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        <InputField id="printWidth" label="Print Width" value={printWidth} onChange={setPrintWidth} unit="in" min={0.1} step={0.1} />
        <InputField id="printHeight" label="Print Height" value={printHeight} onChange={setPrintHeight} unit="in" min={0.1} step={0.1} />
        <div className="space-y-2">
          <InputField id="dpi" label="DPI" value={dpi} onChange={setDpi} min={1} />
          <div className="flex flex-wrap gap-1">
            {COMMON_DPI.map((d) => (
              <button key={d} onClick={() => setDpi(d.toString())} className="text-xs px-2 py-1 rounded border hover:bg-muted/50">{d}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {COMMON_PRINT_SIZES.slice(0, 6).map((size) => (
          <button
            key={size.name}
            onClick={() => { setPrintWidth(size.width.toString()); setPrintHeight(size.height.toString()); }}
      ...
      {result && (
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-lg border bg-muted/50">
            ...
            <p className={`text-xl font-semibold ${getQualityColor(result.printQuality)}`}>{result.printQuality}</p>
