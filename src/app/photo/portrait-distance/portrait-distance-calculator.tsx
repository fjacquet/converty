...
import { calculatePortraitDistance, PORTRAIT_TYPES, PORTRAIT_FOCAL_LENGTHS, type PortraitType } from "@/lib/converters/photo/portrait-distance";
...
export function PortraitDistanceCalculator() {
  ...
  const result = calculatePortraitDistance(parseFloat(focalLength) || 0, portraitType, parseFloat(cropFactor) || 1);
  ...
  const getCompressionColor = (effect: string) => {
    ...
    if (effect === "Natural" || effect === "Flattering") return "text-green-600 dark:text-green-400";
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="space-y-2">
          ...
          <select value={portraitType} onChange={(e) => setPortraitType(e.target.value as PortraitType)} className="w-full h-10 px-3 rounded-md border bg-background">
            {PORTRAIT_TYPES.map((pt) => (<option key={pt.id} value={pt.id}>{pt.name}</option>))}
          </select>
        </div>
        <div className="space-y-2">
      ...
      {result && (
        <div className="space-y-6">
          <OutputDisplay label="Recommended Distance" value={`${result.recommendedDistance} m`} size="lg" />
          ...
          <div className="p-4 rounded-lg border bg-muted/50">
            ...
            <p className={`text-xl font-semibold ${getCompressionColor(result.compressionEffect)}`}>{result.compressionEffect}</p>
            <p className="text-sm text-muted-foreground mt-2">{result.description}</p>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b"><th className="text-left py-2">Focal Length</th><th className="text-right py-2">Distance</th><th className="text-left py-2 pl-4">Effect</th></tr></thead>
              <tbody>
                {PORTRAIT_FOCAL_LENGTHS.map((fl) => {
                  const r = calculatePortraitDistance(fl, portraitType, parseFloat(cropFactor) || 1);
                  return (
                    <tr key={fl} className={`border-b border-muted ${fl === parseFloat(focalLength) ? "bg-primary/10" : ""}`}>
                      <td className="py-2 font-mono">{fl}mm</td>
                      <td className="py-2 text-right font-mono">{r?.recommendedDistance} m</td>
                      <td className={`py-2 pl-4 ${getCompressionColor(r?.compressionEffect || "")}`}>{r?.compressionEffect}</td>
                    </tr>
                  );
                })}
              </tbody>
