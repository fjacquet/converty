...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "categories.other" });
...
export default async function OtherPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations("categories.other");
  const tc = await getTranslations("converters");

  const category = getCategoryBySlug("other")!;
  const converters = getConvertersByCategory("other");

  // Group converters by subcategory
  const subcategories = ["education", "construction", "tech", "weather", "automotive", "engineering", "science", "games"];
  const grouped = subcategories.map((sub) => ({
    id: sub,
    converters: converters.filter((c) => c.subcategory === sub),
  })).filter((g) => g.converters.length > 0);

  // If no converters yet, show coming soon message
    ...
    return (
      <div className="container py-10">
        ...
        <Card className="p-8 text-center">
          <p className="text-muted-foreground">Calculators coming soon...</p>
        </Card>
      </div>
    );
  }
        ...
        <div key={group.id} className="mb-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {group.converters.map((converter) => (
              <Link key={converter.id} href={`/other/${converter.slug}`}>
