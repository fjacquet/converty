...
describe("calculatePace", () => {
  describe("null for invalid inputs (mode: pace)", () => {
    it("returns null for distance <= 0", () => {
      expect(
        calculatePace({ mode: "pace", distance: 0, hours: 0, minutes: 50, seconds: 0 })
      ).toBeNull();
    });

    it("returns null when total time is 0", () => {
      expect(
        calculatePace({ mode: "pace", distance: 10, hours: 0, minutes: 0, seconds: 0 })
      ).toBeNull();
    });
  });
  ...
  describe("mode: pace — 10km in 50 minutes", () => {
    it("calculates pace as 5:00/km", () => {
      ...
      expect(result!.pace.minutes).toBe(5);
    ...
    it("calculates speed as 12 km/h", () => {
      ...
      expect(result).not.toBeNull();
      expect(result!.speed).toBeCloseTo(12, 1);
    });
    ...
    it("returns pace per mile (1 mile = 1.60934 km)", () => {
      ...
      expect(result!.paceMile.minutes).toBe(8);
      expect(result!.paceMile.seconds).toBe(3);
    });
  });
  ...
  describe("mode: time — calculate time from distance and pace", () => {
    it("returns null when distance <= 0", () => {
      expect(
        calculatePace({ mode: "time", distance: 0, paceMinutes: 5, paceSeconds: 0 })
      ).toBeNull();
    });
    ...
    it("calculates total time for 10km at 5:00/km pace", () => {
      ...
      expect(result).not.toBeNull();
      expect(result!.totalTime.hours).toBe(0);
      expect(result!.totalTime.minutes).toBe(50);
      expect(result!.totalTime.seconds).toBe(0);
    });
  });
  ...
  describe("splits array", () => {
    it("returns splits for each km", () => {
      ...
      expect(result).not.toBeNull();
      expect(result!.splits.length).toBeGreaterThanOrEqual(5);
    });
  });
});
