...
describe("calculatePermutationCombination", () => {
  it("returns null for negative n", () => {
    expect(calculatePermutationCombination({ mode: "permutation", n: -1, r: 2 })).toBeNull();
  });

  it("returns null for negative r", () => {
    expect(calculatePermutationCombination({ mode: "combination", n: 5, r: -1 })).toBeNull();
  });

  it("returns null for permutation when r > n", () => {
    expect(calculatePermutationCombination({ mode: "permutation", n: 3, r: 5 })).toBeNull();
  });

  it("returns null for combination when r > n", () => {
    expect(calculatePermutationCombination({ mode: "combination", n: 3, r: 5 })).toBeNull();
  });
  ...
  it("calculates combination with repetition", () => {
    ...
    expect(result).not.toBeNull();
    expect(result!.result).toBe(15); // C(5+2-1, 2) = C(6,2) = 15
  });
});
