...
describe("calculateTriangle", () => {
  describe("sides mode (SSS)", () => {
    it("computes 3-4-5 right triangle correctly", () => {
      ...
      const angleSum = result!.angleA + result!.angleB + result!.angleC;
  ...
  describe("sasAngle mode (SAS)", () => {
    it("law of cosines: a=7, b=8, C=60° computes c correctly", () => {
      ...
      expect(result?.sideC).toBeCloseTo(Math.sqrt(57), 3);
    });
  ...
  describe("asaAngles mode (ASA)", () => {
    it("calculates third side from two angles and included side", () => {
      ...
      expect(result?.sideA).toBeCloseTo(result!.sideC, 3);
      expect(result?.sideB).toBeCloseTo(result!.sideC, 3);
    });
  ...
  describe("triangle properties", () => {
    it("calculates perimeter and area for 3-4-5 triangle", () => {
      ...
      expect(result?.area).toBeCloseTo(6, 5); // (1/2) * 3 * 4 = 6
    });
    ...
    it("calculates inradius and circumradius", () => {
      ...
      expect(result?.inradius).toBeCloseTo(1, 5);
      // circumradius of 3-4-5 = c/2 = 5/2 = 2.5
      expect(result?.circumradius).toBeCloseTo(2.5, 5);
    });

    it("calculates altitudes", () => {
      const result = calculateTriangle({ mode: "sides", sideA: 3, sideB: 4, sideC: 5 });
      expect(result).not.toBeNull();
      expect(result?.altitudeA).toBeGreaterThan(0);
      expect(result?.altitudeB).toBeGreaterThan(0);
      expect(result?.altitudeC).toBeGreaterThan(0);
    });

    it("calculates medians", () => {
      const result = calculateTriangle({ mode: "sides", sideA: 3, sideB: 4, sideC: 5 });
      expect(result).not.toBeNull();
      expect(result?.medianA).toBeGreaterThan(0);
      expect(result?.medianB).toBeGreaterThan(0);
      expect(result?.medianC).toBeGreaterThan(0);
    });

    it("semiperimeter = perimeter / 2", () => {
      const result = calculateTriangle({ mode: "sides", sideA: 3, sideB: 4, sideC: 5 });
      expect(result).not.toBeNull();
      expect(result?.semiperimeter).toBeCloseTo(6, 5);
    });
  });
