...
describe("calculateWindowsLicensing", () => {
  describe("null returns for invalid inputs", () => {
    it("returns null for hostCount = 0", () => {
      expect(calculateWindowsLicensing({ ...BASE_INPUT, hostCount: 0 })).toBeNull();
    });

    it("returns null for coresPerCpu = 0", () => {
      expect(calculateWindowsLicensing({ ...BASE_INPUT, coresPerCpu: 0 })).toBeNull();
    });

    it("returns null for socketsPerHost = 0", () => {
      expect(calculateWindowsLicensing({ ...BASE_INPUT, socketsPerHost: 0 })).toBeNull();
    });

    it("returns null for vmCount = 0", () => {
      expect(calculateWindowsLicensing({ ...BASE_INPUT, vmCount: 0 })).toBeNull();
    });
  });
  ...
  describe("datacenter edition", () => {
    ...
    it("datacenter totalCost is positive", () => {
      const result = calculateWindowsLicensing(BASE_INPUT);
      expect(result!.datacenter.totalCost).toBeGreaterThan(0);
    });

    it("datacenter vmsIncluded is 'Unlimited'", () => {
      const result = calculateWindowsLicensing(BASE_INPUT);
      expect(result!.datacenter.vmsIncluded).toContain("nlimited");
    });
  });
  ...
  describe("standard edition", () => {
    it("standard totalCost is positive", () => {
      const result = calculateWindowsLicensing(BASE_INPUT);
      expect(result!.standard.totalCost).toBeGreaterThan(0);
    });

    it("standard has vmsIncluded (limited number)", () => {
      const result = calculateWindowsLicensing(BASE_INPUT);
      expect(result!.standard.vmsIncluded).toBeGreaterThan(0);
    });
  });
  ...
  describe("comparison", () => {
    it("recommendation is datacenter or standard", () => {
      const result = calculateWindowsLicensing(BASE_INPUT);
      expect(["datacenter", "standard"]).toContain(result!.comparison.recommendation);
    });

    it("breakEvenVms is positive", () => {
      const result = calculateWindowsLicensing(BASE_INPUT);
      expect(result!.comparison.breakEvenVms).toBeGreaterThan(0);
    });
    ...
    it("datacenter is better for high VM counts (many VMs)", () => {
      ...
      expect(result!.comparison.recommendation).toBe("datacenter");
    });
  ...
  describe("more hosts → higher cost", () => {
    it("4 hosts costs more than 1 host (same config)", () => {
      ...
      expect(fourHosts!.datacenter.totalCost).toBeGreaterThan(oneHost!.datacenter.totalCost);
    });
  });
