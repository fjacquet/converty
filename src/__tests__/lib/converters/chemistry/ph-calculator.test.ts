...
describe("calculatePh (real acids-bases.json data)", () => {
  describe("null returns for invalid inputs", () => {
    it("returns null for strong-acid with concentration = 0", () => {
      expect(
        calculatePh({
          mode: "strong-acid",
          concentration: 0,
    ...
    it("returns null for from-h-concentration with hConcentration = 0", () => {
      expect(
        calculatePh({
    ...
    it("returns null for from-ph with ph outside 0-14 range", () => {
      expect(
        calculatePh({
          mode: "from-ph",
          ph: -1,
    ...
    it("returns null for from-ph with ph > 14", () => {
      expect(
        calculatePh({
          mode: "from-ph",
          ph: 15,
  ...
  describe("strong-acid mode", () => {
    it("strong acid HCl 0.01 M → pH = 2.0", () => {
      ...
      expect(result!.ph).toBeCloseTo(2.0, 1);
    });
    ...
    it("strong acid 1.0 M → pH = 0.0", () => {
      ...
      expect(result).not.toBeNull();
      expect(result!.ph).toBeCloseTo(0.0, 1);
    });
  });
  ...
  describe("strong-base mode", () => {
    it("strong base NaOH 0.01 M → pH = 12.0", () => {
      ...
      expect(result).not.toBeNull();
      expect(result!.ph).toBeCloseTo(12.0, 1);
    });
  });
  ...
  describe("result structure", () => {
    it("result includes ph, poh, hConcentration, ohConcentration, color, steps", () => {
      ...
      expect(result!.poh).toBeDefined();
      expect(result!.hConcentration).toBeDefined();
      expect(result!.ohConcentration).toBeDefined();
    ...
    it("pH + pOH = 14", () => {
      ...
      expect(result!.ph + result!.poh).toBeCloseTo(14, 5);
    });
  });
});
