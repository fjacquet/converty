...
export function calculateBandwidthDelayProduct(input: BandwidthDelayProductInput): BandwidthDelayProductResult {
  ...
  steps.push(`Window Size = ${windowSize} KB = ${windowSize * 1024} bytes = ${windowBits.toLocaleString()} bits`);
  ...
  steps.push(`Maximum Throughput = ${maxThroughputBps.toLocaleString()} bps = ${maxThroughputMbps.toFixed(2)} Mbps`);
  ...
  steps.push(`Window Utilization = (${windowSize} KB / ${requiredWindowKB.toFixed(2)} KB) × 100 = ${windowUtilization.toFixed(1)}%`);
  ...
  if (!isWindowSufficient) {
    ...
    recommendations.push(`Current window size is ${deficit.toFixed(2)} KB smaller than required for full bandwidth utilization.`);
  ...
  if (rtt > 100) {
    recommendations.push("High RTT: Consider enabling TCP window scaling for long-distance connections.");
  }

  if (bandwidth > 100 && windowSize < 256) {
    recommendations.push("High bandwidth with small window: Enable TCP window scaling (RFC 1323) for optimal performance.");
  }

  if (windowUtilization < 50) {
    recommendations.push(`Only ${windowUtilization.toFixed(0)}% of bandwidth can be utilized with current settings.`);
  }
